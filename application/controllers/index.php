...
class Index extends CI_Controller{   
    ...
    public function index(){
        ...
        {            
            //May-tinh-d1                
            $data['linkToDepartment'][]=$this->link->ToDepartment($a['name'],$a['department_id']);
        }
        ...
        if ($this->uri->segment(1) !== FALSE ||
            ...
            if($this->handle_string->getPage($this->uri->segment(1))!=0)
            //cái này wan trọng cho phân trang nghen em
                $this->mPage=$this->handle_string->getPage($this->uri->segment(1));
        ...
        if($this->mSelectedDepartment >0){
            ...
            {                 
                 //duong dan
                //May-tinh-d1                
                $data['linkToCategory'][]=$this->link->ToCategory($this->mSelectedDepartmentName,$this->mSelectedDepartment,$a['name'],$a['category_id']);
            }                                        
        ...
        for ($i = 1; $i <= $this->mrTotalPages; $i++)
            if ($this->mSelectedCategory>0)
                ...
                $this->link->ToCategory($this->mSelectedDepartmentName,$this->mSelectedDepartment,$this->mSelectedCategoryName,$this->mSelectedCategory).'/page-'.$i;
            elseif ($this->mSelectedDepartment>0)
                $this->mProductListPages[] =
                $this->link->ToDepartment($this->mSelectedDepartmentName,$this->mSelectedDepartment).'/page-'.$i;
            else
              $this->mProductListPages[] =  $this->link->ToIndex($i);
        $data['productListPages']=$this->mProductListPages;                      
